    ...
    {tab === 'projects' && <div className="business-project-layout">
      ...
      <section className="business-project-main glass-panel">{selectedProject ? <>
        ...
        <div className="business-project-grid">
          ...
          <section><div className="business-pane-head"><span>评论与交接</span><MessageSquarePlus size={14} /></div><div className="business-comments">{comments.slice(0, 6).map((comment) => <article key={comment.id}><strong data-i18n-ignore="true">{comment.ownerId}</strong><p data-i18n-ignore="true">{String(comment.data.body ?? '')}</p><small>{fmtTime(comment.updatedAt)}</small></article>)}{comments.length === 0 && <small>还没有评论，可用 @账号 提及成员。</small>}</div><div className="business-inline-editor"><input value={commentDraft} onChange={(event) => setCommentDraft(event.target.value)} placeholder="评论或 @成员" onKeyDown={(event) => { if (event.key === 'Enter') addComment(); }} /><button type="button" onClick={addComment} disabled={!commentDraft.trim() || busy}><ChevronRight size={14} /></button></div></section>
    ...
    {tab === 'tools' && <section className="business-section glass-panel">
      ...
      <div className="business-capability-scroll">
        ...
        return <article key={source.id} className={`tool-health-${health}${pendingAuthorization ? ' tool-auth-pending' : ''}`}>
          ...
          <p data-i18n-ignore="true">{source.data.description || source.data.endpoint}</p>
          <div className="business-tool-capabilities">{(source.data.capabilityTags ?? []).slice(0, 6).map((name) => <span key={name} data-i18n-ignore="true">{name}</span>)}{(source.data.capabilityTags ?? []).length === 0 && <span>系统会根据工具说明自动识别能力</span>}</div>
