...
export function ShellHeader({ phase, provider, theme, onThemeChange, onNewTask, onOpenSettings, onBack }: { phase: AgentPhase; provider: string; theme: UiTheme; onThemeChange: (theme: UiTheme) => void; onNewTask: () => void; onOpenSettings: () => void; onBack: () => void }) {
  return <header className="shell-header">
    ...
    <div className="shell-header-actions"><button type="button" title="系统状态" aria-label="系统状态"><CircleHelp size={16} /></button><button type="button" title="运行设置" aria-label="运行设置" onClick={onOpenSettings}><Settings2 size={16} /></button><ThemePicker value={theme} onChange={onThemeChange} /><button type="button" title="返回任务台" aria-label="返回任务台" onClick={onBack}><ArrowLeft size={16} /></button></div>
  </header>;
}
