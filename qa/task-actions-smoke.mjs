...
`;
  ...
  await check('denying a tool is immediate while a critical write retains its final confirmation', async () => {
    tasks.set('critical', { ...task('critical', 'waiting_for_human'), toolApprovals: [{ id: 'critical-write', name: 'external.delete', args: { recordId: 'fixture' }, status: 'pending', risk: 'critical' }] });
    await page.evaluate(() => window.qa.mount('panel', 'critical'));
    ...
    assert.equal(mutations.at(-1).action, 'reject-tool');
    assert.equal(mutations.at(-1).body.expectedRevision, 4);
  });
