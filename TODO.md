...
- [ ] 复合附件路由保留模型选定的多 Agent 计划，附件作为必需能力而非互斥意图。
- [ ] 分离执行完成、人工接受和证据核验；模型自报 verification 不能直接作为已验证事实。
- [ ] 通用 Tool Registry 持久化执行账本、恢复重放和结果未知处置，区分恢复与用户有意重复操作。
- [ ] Agent 工具多轮观察、决策、行动、验证闭环；进展检测、终止条件和有界预算。
- [ ] 长对话结构化约束与决定保留、原文检索和修改/撤销语义回归。

第三批：产品质量。

- [ ] 有数据的双语状态、用户命名保护、动态 ARIA 文案和移动端回归。
- [ ] 扩展复合任务与跨轮路由评测，增加引用正确率、任务完成率和人工接管统计。
