...
#### 2026-09-07 跨入口执行与人工协作一致性（本地验收完成，待同步）

本批先交付前四项 P0 与运行观测/运行测试“需要处理”栏样式统一，完成验收后同步 Gitee 和 GitHub。改动前完整工作树快照：`frontend-backup/20260907-pre-cross-entry-consistency.zip`。普通对话与 Agent Nexus 历史继续隔离。
...
- [ ] 发布同步：README/中文说明/变更记录与版本一致，检查部署包文档和敏感文件，提交后将同一提交推送到 Gitee/GitHub 并核对远端。

后续产品质量项（不混记为本批完成）：

- [ ] P0 路由降级边界：Router 不可用时，检索词不能吞掉复合任务的分析、验证和交付需求；统一服务端与浏览器兜底策略，保留天气/简单搜索的轻量路径，并用本批失败的 PostgreSQL/SQLite 官方资料对比输入回归。
- [ ] 有数据的双语状态、用户命名保护、动态 ARIA 文案和移动端回归。
- [ ] 扩展复合任务与跨轮路由评测，增加引用正确率、任务完成率、路由降级率、首次通过率和人工接管统计。
- [ ] 真实工作负载延迟/内存/3D 帧耗时门槛；淘汰脱离实际渲染器的旧专项。
