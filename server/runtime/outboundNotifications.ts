...
type DeliveryRow = {
  ...
  lease_expires_at: unknown; created_at: unknown; updated_at: unknown;
};
...
const publicDelivery = ({ tenantId: _tenantId, userId: _userId, payload: _payload, leaseOwner: _leaseOwner, leaseExpiresAt: _leaseExpiresAt, ...delivery }: StoredDelivery): OutboundNotificationDelivery => delivery;
...
export interface OutboundNotificationStore {
  ...
  markDelivered(id: string, workerId: string, responseStatus: number): Promise<void>;
  markFailed(id: string, workerId: string, error: string, responseStatus?: number, terminal?: boolean): Promise<void>;
...
export class SqliteOutboundNotificationStore implements OutboundNotificationStore {
  ...
  async initialize() {
    this.db.exec(`
      ...
      CREATE TABLE IF NOT EXISTS outbound_notification_deliveries (
        ...
        delivered_at TEXT, response_status INTEGER, last_error TEXT, lease_owner TEXT, lease_expires_at TEXT,
  ...
  async claimNext(workerId: string, leaseMs: number, id?: string) {
    ...
    try {
      const row = this.db.prepare(`SELECT * FROM outbound_notification_deliveries
        WHERE status IN ('pending', 'retrying') AND next_attempt_at <= ? AND (lease_expires_at IS NULL OR lease_expires_at <= ?)
      ...
      this.db.prepare(`UPDATE outbound_notification_deliveries SET status = 'delivering', lease_owner = ?, lease_expires_at = ?, last_attempt_at = ?, updated_at = ? WHERE id = ?`)
        .run(workerId, leaseExpiresAt, nowIso, nowIso, String(row.id));
      this.db.exec('COMMIT');
      return deliveryFromRow({ ...row, status: 'delivering', lease_owner: workerId, lease_expires_at: leaseExpiresAt, last_attempt_at: nowIso, updated_at: nowIso });
    } catch (error) {
  ...
  async markDelivered(id: string, workerId: string, responseStatus: number) {
    const now = new Date().toISOString();
    this.db.prepare(`UPDATE outbound_notification_deliveries SET status = 'delivered', attempt_count = attempt_count + 1,
      total_attempts = total_attempts + 1, response_status = ?, delivered_at = ?, last_error = NULL, lease_owner = NULL,
      lease_expires_at = NULL, updated_at = ? WHERE id = ? AND lease_owner = ?`).run(responseStatus, now, now, id, workerId);
  }

  async markFailed(id: string, workerId: string, error: string, responseStatus?: number, terminal = false) {
    const row = this.db.prepare('SELECT attempt_count FROM outbound_notification_deliveries WHERE id = ? AND lease_owner = ?').get(id, workerId) as { attempt_count?: number } | undefined;
    if (!row) return;
    const attempts = Number(row.attempt_count ?? 0) + 1;
    const dead = terminal || attempts >= 5;
    const now = new Date();
    const delayMs = Math.min(60 * 60_000, 5_000 * (2 ** Math.max(0, attempts - 1)));
    this.db.prepare(`UPDATE outbound_notification_deliveries SET status = ?, attempt_count = ?, total_attempts = total_attempts + 1,
      next_attempt_at = ?, response_status = ?, last_error = ?, lease_owner = NULL, lease_expires_at = NULL, updated_at = ?
      WHERE id = ? AND lease_owner = ?`).run(dead ? 'dead_letter' : 'retrying', attempts, new Date(now.getTime() + delayMs).toISOString(), responseStatus ?? null, boundedError(error), now.toISOString(), id, workerId);
  }
  ...
  async retry(id: string, tenantId: string, userId: string) {
    ...
    return this.db.prepare(`UPDATE outbound_notification_deliveries SET status = 'pending', attempt_count = 0, next_attempt_at = ?,
      last_error = NULL, response_status = NULL, lease_owner = NULL, lease_expires_at = NULL, updated_at = ?
...
export class PostgresOutboundNotificationStore implements OutboundNotificationStore {
  ...
  async initialize() {
    await this.pool.query(`
      ...
      CREATE TABLE IF NOT EXISTS outbound_notification_deliveries (
        ...
        last_error TEXT, lease_owner TEXT, lease_expires_at TIMESTAMPTZ, created_at TIMESTAMPTZ NOT NULL, updated_at TIMESTAMPTZ NOT NULL,
        UNIQUE(channel_id, notification_id)
      );
      CREATE INDEX IF NOT EXISTS idx_outbound_deliveries_due ON outbound_notification_deliveries(status, next_attempt_at, lease_expires_at);
      CREATE INDEX IF NOT EXISTS idx_outbound_deliveries_owner ON outbound_notification_deliveries(tenant_id, user_id, created_at DESC);
  ...
  async claimNext(workerId: string, leaseMs: number, id?: string) {
    ...
    try {
      ...
      const result = await client.query(`SELECT * FROM outbound_notification_deliveries WHERE status IN ('pending','retrying')
      ...
      const updated = await client.query(`UPDATE outbound_notification_deliveries SET status='delivering', lease_owner=$1,
        lease_expires_at=$2, last_attempt_at=NOW(), updated_at=NOW() WHERE id=$3 RETURNING *`, [workerId, leaseExpiresAt, result.rows[0].id]);
      await client.query('COMMIT');
      return deliveryFromRow(updated.rows[0] as DeliveryRow);
    } catch (error) { await client.query('ROLLBACK'); throw error; } finally { client.release(); }
  }
  async markDelivered(id: string, workerId: string, responseStatus: number) {
    await this.pool.query(`UPDATE outbound_notification_deliveries SET status='delivered', attempt_count=attempt_count+1,
      total_attempts=total_attempts+1,response_status=$1,delivered_at=NOW(),last_error=NULL,lease_owner=NULL,lease_expires_at=NULL,updated_at=NOW()
      WHERE id=$2 AND lease_owner=$3`, [responseStatus, id, workerId]);
  }
  async markFailed(id: string, workerId: string, error: string, responseStatus?: number, terminal = false) {
    const client = await this.pool.connect();
    try {
      await client.query('BEGIN');
      const result = await client.query('SELECT attempt_count FROM outbound_notification_deliveries WHERE id=$1 AND lease_owner=$2 FOR UPDATE', [id, workerId]);
      if (!result.rows[0]) { await client.query('COMMIT'); return; }
      const attempts = Number(result.rows[0].attempt_count ?? 0) + 1;
      const dead = terminal || attempts >= 5;
      const delayMs = Math.min(60 * 60_000, 5_000 * (2 ** Math.max(0, attempts - 1)));
      await client.query(`UPDATE outbound_notification_deliveries SET status=$1,attempt_count=$2,total_attempts=total_attempts+1,
        next_attempt_at=$3,response_status=$4,last_error=$5,lease_owner=NULL,lease_expires_at=NULL,updated_at=NOW() WHERE id=$6 AND lease_owner=$7`,
      [dead ? 'dead_letter' : 'retrying', attempts, new Date(Date.now() + delayMs).toISOString(), responseStatus ?? null, boundedError(error), id, workerId]);
      await client.query('COMMIT');
    } catch (error) { await client.query('ROLLBACK'); throw error; } finally { client.release(); }
  }
  async retry(id: string, tenantId: string, userId: string) {
    const result = await this.pool.query(`UPDATE outbound_notification_deliveries SET status='pending',attempt_count=0,next_attempt_at=NOW(),
      last_error=NULL,response_status=NULL,lease_owner=NULL,lease_expires_at=NULL,updated_at=NOW()
...
export class OutboundNotificationManager {
  ...
  private async deliver(delivery: StoredDelivery) {
    ...
    try {
      ...
      await this.store.markFailed(delivery.id, this.workerId, `通知渠道无法解密：${boundedError(error)}`);
      return;
    }
    if (!channel || !channel.enabled) {
      await this.store.markFailed(delivery.id, this.workerId, '通知渠道不存在或已停用。', undefined, true);
      return;
    }
    try {
      ...
      if (response.ok) {
        await this.store.markDelivered(delivery.id, this.workerId, response.status);
        return;
      }
      const terminal = response.status >= 400 && response.status < 500 && ![408, 409, 425, 429].includes(response.status);
      await this.store.markFailed(delivery.id, this.workerId, `Webhook 返回 HTTP ${response.status}。`, response.status, terminal);
    } catch (error) {
      await this.store.markFailed(delivery.id, this.workerId, boundedError(error));
    }
  }
}
