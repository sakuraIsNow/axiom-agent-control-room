...
- 为 DeepSeek Harness 或 Codex app-server 固定 sidecar 版本、命令、工作区和审批策略，使用真实 Thread/Turn/Subscribe 流验证审批回放、Artifact lineage、断点恢复和异常断流重连。
- 在 Agent Nexus 建立条件分支、独立 Loop 和嵌套 Loop 业务 case，验证 branch/loop 事件顺序、局部 rerun 只重跑 descendants，并记录任务成功率、人工接管率和成本。

### 5. 质量、评测与运营

- 参考 HarnessEval-W 建立 case-specific routing、证据树、父任务验证和跨版本回归集；当前已有控制流和路由协议回归，但仍不能代表真实业务准确率。
