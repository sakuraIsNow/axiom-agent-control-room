...
9 月 6 日审计发现了现有测试未覆盖的默认熔断恢复、通知租约接管、日程并发修改、旧历史关联操作和前端交互问题。第一批修复已按[稳定性升级记录](stability-upgrade-20260906.md)验收：标准检查与构建通过，单测 457 passed / 0 failed / 7 skipped，全部 7 个 PostgreSQL 用例在隔离专项 9/9 中补跑通过；MCP 44/44、前端稳定性 7/7、视觉 170/170 通过。下文 9 月 4 日及更早的通过结果是历史基线，不代表整个企业生产系统已经验收。完整 `qa:all:local` 本轮未重跑；复合附件路由、证据可信度、通用工具执行闭环和长摘要质量仍在下一批 TODO 中。
