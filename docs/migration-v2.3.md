...
本说明覆盖执行闭环与跨入口协作候选版。升级前先停止接收新任务，等待正在执行的外部写入结束；保存数据库、Artifact 存储和部署配置的备份。不要通过删除数据库解决迁移问题。
...
运行 `npm run check`、`npm test`、`npm run build`、`npm run qa:visual` 和 `npm run qa:all:local`。完整门禁使用独立 PostgreSQL 测试库，不应指向业务库执行破坏性测试。测试结果与外部依赖边界见 [本批记录](cross-entry-consistency-20260907.md)。
