...
Nexus 可以上传持久附件，也可以链接当前租户 Artifact 目录中仍有效的结果。新上传文件以原始二进制保存，不再转成 Base64 Markdown；附件记录固定工作流 ID、版本、MIME、字节数、SHA-256、存储键和编码方式。下载、测试、发布和运行时都会再次校验租户与流程归属，浏览器内存文件不会被当成已经持久化的附件。

每次测试都会固定当时的附件快照和附件集合摘要。发布只接受当前附件集合已经通过的测试；新增、替换或删除附件后，旧测试立即变为过期，必须重新测试。Release 摘要同时覆盖流程定义与附件集合，因此“流程没变但文件变了”不会错误复用旧发布版本。

运行已发布 Nexus 时，视觉 Agent 读取真实图片字节，文档 Agent 读取 PDF、Word、TXT、Markdown、CSV、JSON、XML、HTML 或 SVG 的解析文本；其他 Agent 不会收到附件 Base64。单文件限制、任务总预算和摘要验证在读取模型前完成，内容被替换或跨租户引用时任务会直接拒绝。默认总预算由 `AXIOM_NEXUS_ARTIFACT_BUDGET_BYTES=20971520` 控制，硬上限为 64 MB。
...
发布会固定当前定义、工作流版本、附件快照和内容摘要。运行已发布 Nexus 时，Task API 读取指定发布快照，不跟随之后的草稿或附件变化。历史发布可比较步骤、Agent、连线和附件的增加、删除与修改，并能恢复为新的草稿。发布版本还可以生成固定 `workflowId + workflowVersion` 的 Workflow Plugin，继续复用 Task、Event、SSE、审批和恢复链路。

## 验收

自动化测试覆盖：单 Loop 兼容 ID、多个嵌套 Loop、上一轮回边依赖、条件命中/跳过、受限表达式拒绝任意代码、四种连线传递、二进制附件往返、附件租户隔离、测试与发布附件一致性、视觉/文档真实输入、篡改拒绝、发布差异、恢复、Workflow Plugin、局部重跑前置条件和上游检查点保留。真实对象存储、多 Worker 与真实模型工作流仍需在部署环境补充成功率、人工接管率和成本评测。
