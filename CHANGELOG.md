...
## Unreleased — 业务交付、稳定性与 RSI 对照评测

- 可选 Jev 结构化决策层：支持 `legacy`、`jev-shadow`、`jev-hybrid`，默认保持原路由；高置信且通过目录/依赖校验的 Agent、Skill 选择交给原 LLM Scheduler，其余回到旧 Router，取消不触发回退。
- Jev 配置仅在服务端读取；支持本地密钥文件、明确的超时/体积限制、安全诊断和实际调用计量。本地模型绕过云端决策，支持显式内网域名。文本、视觉、搜索、媒体和原有步骤模型选择策略不变。
- 修复附件 Agent 已存在时后续分析缺少依赖的问题，图片/文档/混合附件结果必须先就绪；保留 DAG 环路检查。
- 新增 Jev 离线回归和 26 例显式启用的中文合成评测。真实选中覆盖率为 7/26，最新观察仍有一次响应格式错误；不将弃权、自动回退或小样本精确率包装成普遍正确率。详见 [Jev 接入与验收记录](docs/jev-routing-integration-20260921.md)。

- 为复杂非空计划增加最终交付验收：原文要求提取、最终答案逐项复核、一次无工具文本修订；超界、异常或缺项保留草稿并就地处理，不把模型自评分当事实准确率。
- 交付回执绑定输入、合同、成果和规范化执行上下文；人工接受仅作用于原成果，恢复不重放已完成工具。补齐复杂模板 profile、JSONB 键序恢复、并发指导/暂停/取消，以及旧 Reviewer 高分与缺口矛盾时的阻断。
- 对话、任务、Nexus 与 Mini App 共享交付检查、双语真实阶段进度和部分交付历史。执行完整性、模型语义复核、人工接受与事实验证分开保存和展示。
- 增加独立判定的两例真实复杂任务检查 `qa:delivery-live`。保留首轮字段歧义和后续 `600/120=1.25` 被模型误判通过的真实失败；补来源绑定的服务端 JSON 数值计算校验，不能仅靠模型自评放行错误算术。不将两例样本推广为任意任务准确率。详见 [最终交付验收记录](docs/final-delivery-verification-20260921.md)。

- 修复服务中断将未完成步骤持久化为失败的恢复缺陷：保留完成步骤和并行成果，被打断的步骤等待恢复；未知副作用继续进入核对，超时不被改成正常中断。
- 增加 24 个跨组件业务交付门禁，覆盖来源报告、多格式附件、HTML/SVG Artifact、插件版本、Nexus 控制流和对话恢复；确定性替身与真实模型质量分开报告。
- 增加显式启用的四例真实模型评测、独立 JSON/SVG 判定器、限额与临时存储隔离。首轮失败和测试器隔离缺陷保留在验收记录，不覆写为一次通过。
- 严格限定的首轮纯对话可由 Router 直接完成调度决策；复杂任务、附件、历史上下文、低置信和纠错路径继续调用 Scheduler。诊断明确区分真实调用与省略阶段。
- 增加无密钥 CI 和 GitHub Actions 双系统配置；门禁保留每次尝试，重试后成功标记为不稳定并非零退出。GitHub 托管环境须推送触发后才能声明已验收。
- 更新就绪度、双语说明与过期 TODO，部署现场验收和私人空间继续独立排期。详见 [业务交付收口记录](docs/business-delivery-closure-20260921.md)。
