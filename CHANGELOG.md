...
## Unreleased — 路由可靠性与 RSI 对照评测

- Router/Scheduler 分离能力描述与可选 ID，共享一次语义纠错额度；校验真实目录、候选集和依赖，空目录不扩权，取消不触发降级。
- `/api/chat/route` 分开返回模型调用与语义校验诊断、首次有效性和纠错开销；服务端整次路由 45 秒预算，浏览器 50 秒边界，不在未知网络结果后重复发起调度。
- 任务改进增加显式启动/停止的持久基线与候选对照：五类固定合成文本场景、配对事实/要求/引用检查、原文与耗时/Token 记录、幂等与失效恢复。
- 新的毛玻璃对照区保留中英文、移动端布局和评测历史；结果仅对样例有效，不自动改写建议全局验证状态或执行策略。
- 保持正式版本 `2.3.0-rc.8`；不包含可信登录、私人空间、真实外部服务现场验收或全业务质量提升声明。

## [2.3.0-rc.8] - 2026-09-20

### 任务改进：受控 RSI 第一期

- 新增“任务改进 / Task improvements”：用户主动复盘自己的终态任务，结合当前任务反馈生成改进建议、参考证据和待执行的验证场景。
- 建议按所有者隔离、持久化并支持保存/忽略、revision 冲突检测、幂等生成和失败恢复；可引用上一版建议，最多五代。
- 已保存建议可带入新对话草稿，用户检查后再发送；不改动现有 Graph、路由、长期记忆、Nexus、插件发布或工具权限。
- 所有建议标为效果未验证；不把模型建议、结构校验或用户保存冒充真实业务质量提升。详见 [受控 RSI](docs/controlled-rsi.md)。
- 来源列表在数据库 LIMIT 前按当前所有者和终态过滤；SQLite/PostgreSQL 共用建议记录契约，模型调用无工具执行，不自动写入记忆或平台执行策略。
- 新对话草稿不自动发送；已有未发送文字或附件时先提示保留。误忽略的建议可直接重新保存，无需再次调用模型。

### 独立成果交付与审批体验

- 新增 `artifact.create` 生成 HTML、SVG、Markdown、TXT 独立任务成果，不为普通作品生成增加工作区写入审批；修改项目文件等原有高风险动作仍遵守原权限与审批规则。
- 成果回执保存真实文件引用，重放复用既有成果；汇总遗漏交付链接时依据真实回执补齐，不恢复已删除或过期的文件。
- 文件预览和下载校验租户及任务所有者/管理员，HTML/SVG 使用隔离 iframe；审批事件即时更新聊天处理入口，critical 操作保留最后确认。
- 修复 PostgreSQL 审批对象数组的 JSONB 序列化，覆盖待审批、批准、清空及 null 状态。详见 [9 月 8 日修复记录](docs/fixes/2026-09-08-artifact-approval.md)。

### 对话预览稳定性

- 稳定 Markdown 渲染组件身份，滚动、输入、后续流式文字和主题切换不再重建内容未变化的 HTML/SVG、文件或媒体预览。
- HTML/SVG 代码围栏完成后才加载，裸 HTML/SVG 等待消息完成；支持嵌套列表、引用块、CRLF 和伪结束围栏判断，避免反复执行半成品。
- 普通对话与 Agent Nexus 的消息身份包含会话/流程范围，保留本会话互动状态，切换会话时不串用旧预览。复制、下载、更新、重试和隔离规则保持有效。详见 [9 月 9 日修复记录](docs/fixes/2026-09-09-preview-stability.md)。

### 验收与升级

- 真实 Agent 目录问答调整为更简洁的概览：普通能力查询聚焦相关角色，用户明确要求完整清单或技术细节时仍保留详细回答；继续由模型根据实时目录作答，不替换为固定介绍。
- 修复英文 Readiness 对新模型名称、`artifact.create` 和更新后 `workspace.write` 说明的漏翻译；动态模型标识与用户自定义工具名称保持原样，审批和权限边界不因翻译改变。
- Hono 更新至 `4.13.5` 安全补丁版本并同步依赖锁文件；部署时使用 `npm ci` 安装锁定版本，不把依赖更新当作完整公网安全验收。
- 新增独立干净安装验证脚本 `scripts/release-repro-smoke.mjs`，分别检查导出源码与已解压部署包的依赖安装、构建产物和只读启动路径；拒绝真实工作区与携带用户配置/数据的副本，不使用真实模型 Key。
- 修正测试样例的环境隔离，避免运行机器的部署配置影响预期；视觉回归等待固定的已路由阶段，再精确检查对应 Agent 动作，消除仅凭英文 `Agent` 字样判断中文角色的时序误报，不放宽业务验收标准。
- 人工审批弹窗夹具明确使用 `critical` 风险动作，普通工具保持一次明确授权即可提交；保留普通授权/拒绝的独立回归，不把旧测试对第二次确认的要求变回产品负担。
- Graph 性能检查用 `finally` 清理遥测、指针监听、CDP 与全屏面板，防止性能断言失败级联污染后续翻译检查；事件测试显式等待样例加载。保留原性能阈值和精确翻译断言，不将测试清理修复宣称为产品性能提升。
- `qa:improvements`、`qa:preview-stability` 和成果/审批回归进入完整生产门禁；受控 RSI PostgreSQL 用例纳入隔离数据库阶段，不向日常任务历史写入测试样例。
- 最终完整门禁 `41 passed / 0 failed / 3 skipped`，最后一整轮全部可运行项首次通过、无重试；单测 `700 tests / 684 passed / 0 failed / 16 PostgreSQL skipped`，隔离 PostgreSQL 专项 `18/18` 补齐。视觉 `172/172`、真实 Graph `22/22`、RSI 浏览器 `17/17`、Fake MCP `44/44`、路由故障 `137/137`、在线路由 `7/7`、固定交付 `14/14`，中英文界面通过。此前失败、性能波动和修复依据保留在 [rc.8 发布验收记录](docs/rc8-release-acceptance-20260920.md)，不将整个过程表述为一次通过。
- 干净源码的锁定安装、检查、测试、构建及隔离 API 启动通过；部署 ZIP 解压后的 `npm ci --omit=dev` 和隔离启动通过，具体检查与边界见发布记录。生产依赖审计为检查时 `0` 项已知漏洞，不等于完整安全审计。
- 三项外部跳过仍为 MemoryCore HTTP、MemoryCore Axiom 适配器和 Harness/Codex sidecar 真实验收，未配置项不计为通过；受控 RSI 测试不证明候选建议在真实业务中优于基线。
- 不增加外部服务要求，建议复用现有业务记录表；保留原凭据加密密钥，重新构建并重启服务加载 `/api/improvements`。详见 [升级说明](docs/migration-v2.3.md)。
- 本版仍为本地/受控内网候选版，不声明完成公共多用户 SaaS、自动策略优化发布或建议质量独立验证。
